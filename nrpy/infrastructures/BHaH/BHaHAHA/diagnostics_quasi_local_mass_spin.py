...
3. Computing the curl of 'omega_A' to get the source term 'Omega' for a
   2D Poisson equation.
4. Solving the Poisson equation D^2(varpi) = Omega for the scalar potential 'varpi'.
   (Note: This requires a separate elliptic solver to be implemented in C).
5. Defining coordinate rotation generators (one-forms) on the surface.
6. Integrating 'varpi' against the curl of the rotation generators over the
   horizon surface to find the components of the spin vector J_i.
7. Using the magnitude J and the irreducible mass M_irr (from the horizon area) to
...
def register_CFunction_diagnostics_quasi_local_mass_spin(
    ...
    # Step 4: Compute Omega = curl of omega_A. This is the source for the Poisson equation.
    ...
    # Step 5: Define the coordinate rotation generators (one-forms) phi_i_A.
    # Using one-form version from Eq. (23) of Owen et al. (2018): kappa_i = eta_ijk (x^j - x_0^j) dx^k
    # Projection onto surface: (kappa_i)_A = kappa_i(e_A) = eta_ijk (x^j - x_0^j) e_A^k
    LeviCivitaSymbolDDD = ixp.LeviCivitaSymbol_dim3_rank3()
    kappa_iD = ixp.zerorank2(dimension=3) # Index i is component, index A is surface direction
    for i in range(3):
        for A in range(2):
            for j in range(3):
                for k in range(3):
                    kappa_iD[i][A] += LeviCivitaSymbolDDD[i][j][k] * \
                                      (rfm.xx_to_Cart[j] - centroid[j]) * tangent_vectors[k][A]

    # Step 6: Define the integrand for each spin component J_i.
    # J_i = (1/8pi) * Integral [ varpi * (epsilon^{AB} D_A (kappa_i)_B) ] dS
    # Curl of kappa: C_i = epsilon^{AB} D_A (kappa_i)_B
    varpi = sp.Symbol("varpi_gf", real=True)
    kappa_iD_dD = ixp.declarerank3("kappa_iD_dD", dimension=3)
    ...
    for i in range(3):
        ...
        Curl_kappa_i = (1/sp.sqrt(q2det)) * (
            # partial_theta (kappa_i)_phi - Gamma^C_{theta,phi} (kappa_i)_C
            (kappa_iD_dD[i][1][0] - (ChristoffelUDD[0][0][1]*kappa_iD[i][0] + ChristoffelUDD[1][0][1]*kappa_iD[i][1])) -
            # partial_phi (kappa_i)_theta - Gamma^C_{phi,theta} (kappa_i)_C
            (kappa_iD_dD[i][0][1] - (ChristoffelUDD[0][1][0]*kappa_iD[i][0] + ChristoffelUDD[1][1][0]*kappa_iD[i][1]))
        )
        J_integrand[i] = varpi * Curl_kappa_i
    ...
    prefunc = r"""
// Placeholder for a 2D Poisson solver on the sphere for D^2(varpi) = Omega.
// A real implementation would use an iterative method like Gauss-Seidel or SOR,
// or a library like PETSc. This is a non-trivial numerical task.
static void solve_poisson_on_horizon(
      const params_struct *restrict params, REAL *restrict xx[3],
      REAL *restrict aux_gfs) {
  // This is a placeholder. For now, we set varpi = Omega as a mock solution.
  // This is NOT a valid physical solution.
